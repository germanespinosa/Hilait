...
class HilaitTerminal {
  constructor(element) {
    ...
    this.term = new Terminal({allowProposedApi:true,fontFamily:'Cascadia Mono, Consolas, monospace',fontSize:14,lineHeight:1.18,cursorBlink:true,scrollback:20000,screenReaderMode:true,
    ...
    this.term.open(element); this.term.onData(data=>this.onInput?.(data)); this.term.onBinary(data=>this.onBinary?.(btoa(data)));
    ...
    this.observer=new ResizeObserver(()=>this.fit()); this.observer.observe(element);
    this.fit();
  }
  _keys(e){
  ...
  fit(){if(this.element.clientWidth>0&&this.element.clientHeight>0)this.fitAddon.fit();}
  ...
  dispose(){this.observer.disconnect();this.element.removeEventListener('contextmenu',this._context);this.element.removeEventListener('click',this._leftPaste);this.term.dispose();}
}
